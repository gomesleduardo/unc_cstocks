...
# ---- Add style
openxlsx::addStyle(
  wb, sheet = "Total Carbon Stocks", cols = 1:2, rows = 1,
  style = h_style
)
...
for (k in 1:nrow(legend_df)) {
  ...
  df_unc <- get(paste0("results__tc__", legend_df$label[k])) %>% 
    ...
    dplyr::select(
      total_est, total_sd, total_cilow, total_ciupp, Unc
    ) %>% 
    dplyr::rename(
      `TC` = total_est,
      `SE` = total_sd,
  ...
  openxlsx::writeData(
    ...
    x =  paste0(legend_df$name[k], "__", names(df_unc)[1]), 
    startCol = (5 * (k - 1) + 3), startRow = 1, 
    rowNames = FALSE
  )
  ...
  openxlsx::writeData(
    wb, sheet = "Total Carbon Stocks", 
    x =  paste0(legend_df$name[k], "__", names(df_unc)[2]), 
    startCol = (5 * (k - 1) + 4), startRow = 1, 
    rowNames = FALSE
  )
  ...
  openxlsx::writeData(
    wb, sheet = "Total Carbon Stocks", 
    x =  paste0(legend_df$name[k], "__", names(df_unc)[3]), 
    startCol = (5 * (k - 1) + 5), startRow = 1, 
    rowNames = FALSE
  )
  ...
  openxlsx::writeData(
    wb, sheet = "Total Carbon Stocks", 
    x =  paste0(legend_df$name[k], "__", names(df_unc)[4]), 
    startCol = (5 * (k - 1) + 6), startRow = 1, 
    rowNames = FALSE
  )
  ...
  openxlsx::writeData(
    wb, sheet = "Total Carbon Stocks", 
    x =  paste0(legend_df$name[k], "__", names(df_unc)[5]), 
    startCol = (5 * (k - 1) + 7), startRow = 1, 
    rowNames = FALSE
  )
  ...
  # ---- Add style
  openxlsx::addStyle(
    wb, sheet = "Total Carbon Stocks", cols = (5 * (k - 1) + 7), rows = 2:559,
