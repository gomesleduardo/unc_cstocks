...
for (k in 1:nrow(legend_df)) {
  ...
  openxlsx::writeData(
    ...
    startCol = (2 * k + 1), startRow = 1, 
    rowNames = FALSE
  )
  
  # ---- Merge names cells
  openxlsx::mergeCells(
    wb, sheet = "Total Carbon Stocks", cols = (2 * k + 1):(2 * k + 2), rows = 1
  )
  ...
  # ---- Add style
  openxlsx::addStyle(
    wb, sheet = "Total Carbon Stocks", cols = (2 * k + 1):(2 * k + 2), rows = 2,
    style = h_style
  )
  
  # ---- Add style
  openxlsx::addStyle(
    wb, sheet = "Total Carbon Stocks", cols = (2 * k + 2), rows = 3:560,
