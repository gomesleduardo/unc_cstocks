...
# ---- Add a sheet: "Soil Organic Carbon Stocks" ----
openxlsx::addWorksheet(wb, sheetName = "Soil Organic Carbon Stocks")

# ---- Add microregions codes and names
openxlsx::writeData(
  wb, sheet = "Soil Organic Carbon Stocks", x = micro_df, 
...
# ---- Add LU name
openxlsx::writeData(
  wb, sheet = "Soil Organic Carbon Stocks", x = names(micro_df)[1], 
  startCol = 1, startRow = 1, 
  rowNames = FALSE
)

openxlsx::writeData(
  wb, sheet = "Soil Organic Carbon Stocks", x = names(micro_df)[2], 
  startCol = 2, startRow = 1, 
  rowNames = FALSE
)

# ---- Merge names cells
openxlsx::mergeCells(wb, sheet = "Soil Organic Carbon Stocks", cols = 1, rows = 1:2)
openxlsx::mergeCells(wb, sheet = "Soil Organic Carbon Stocks", cols = 2, rows = 1:2)
...
# ---- Add style
openxlsx::addStyle(
  wb, sheet = "Soil Organic Carbon Stocks", cols = 1, rows = 1:2,
  style = h_style
)

openxlsx::addStyle(
  wb, sheet = "Soil Organic Carbon Stocks", cols = 2, rows = 1:2,
  style = h_style
)
...
for (k in 1:nrow(legend_df)) {
  ...
  df_unc <- get(paste0("results__tc__", legend_df$label[k])) %>% 
    dplyr::select(
      soc_est, soc_sd, soc_cilow, soc_ciupp, soc_uncl, soc_uncu
    ) %>% 
    dplyr::mutate(
      soc_est = round(soc_est, 1),
      soc_sd = round(soc_sd, 1),
      soc_cilow = round(soc_cilow, 1),
      soc_ciupp = round(soc_ciupp, 1),
      Unc = ifelse(
        test = soc_uncu > soc_uncl, 
        yes = soc_uncu, 
        no = soc_uncl
      )
    ) %>% 
    dplyr::select(soc_est, soc_sd, soc_cilow, soc_ciupp, Unc) %>% 
    dplyr::rename(
      `SOC` = soc_est,
      `SE` = soc_sd,
      `CI95% Low` = soc_cilow,
  ...
  openxlsx::writeData(
    wb, sheet = "Soil Organic Carbon Stocks", x = df_unc, 
  ...
  # ---- Add LU name
  openxlsx::writeData(
    wb, sheet = "Soil Organic Carbon Stocks", x = legend_df$name[k], 
  ...
  # ---- Merge names cells
  openxlsx::mergeCells(
    wb, sheet = "Soil Organic Carbon Stocks", 
  ...
  # ---- Add style
  openxlsx::addStyle(
    wb, sheet = "Soil Organic Carbon Stocks", cols = (5 * (k - 1) + 7), rows = 3:560,
