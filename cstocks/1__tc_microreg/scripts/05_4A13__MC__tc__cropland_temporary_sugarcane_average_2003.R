...
# SETTINGS --------------------------------------------------------------------
type_name <- 'tc__cropland_temporary_sugarcane_average_2003'
...
fi_name1 <- 'samples__cropland_input_f1_low'
fi_name2 <- 'samples__cropland_input_f1_high_without_manure'
cveg_name <- 'sugarcane'
...
rm(list = ls()[!(ls() %in% c(
  ...
  'fmg_name1', 'fmg_name2', 'fi_name1', 'fi_name2', 'cveg_name',
  'climate__micro', 'unc_cveg_sugarcane', 'harvesting_df'
))])
...
for (x in c(
  socref_name, flu_name1, flu_name2, fmg_name1, fmg_name2, fi_name1, fi_name2
)) {
  load(
    file.path(
...
for (i in 1:nrow(climate__micro_temp)) {
  ...
  dist_fi <- harvesting_df$manual_harvesting_2003[i] * (
    fi_name1 %>% 
      get() %>% 
      magrittr::use_series("samples") %>% 
      apply(1, function(x, w){x * w}, 
      ...
      t() %>% 
      rowSums()
  ) + harvesting_df$mechanized_harvesting_2003[i] * (
    fi_name2 %>% 
      get() %>% 
      magrittr::use_series("samples") %>% 
      apply(1, function(x, w){x * w}, 
