...
# SETTINGS --------------------------------------------------------------------
type_name <- 'tc__cropland_temporary_sugarcane_high'
socref_name <- 'samples__socref'
flu_name1 <- 'samples__cropland_land_use_flu_set_aside_20_years'
flu_name2 <- 'samples__cropland_land_use_flu_annual_cropland'
fmg_name1 <- 'samples__cropland_tillage_fmg_no_till'
fmg_name2 <- 'samples__cropland_tillage_fmg_full'
fi_name <- 'samples__cropland_input_f1_high_without_manure'
cveg_name <- 'sugarcane'
...
# ---- Remove extra data
rm(list = ls()[!(ls() %in% c(
  type_name, 'unc_factors__sugarcane',
  'type_name', 'socref_name', 'flu_name1', 'flu_name2', 
  'fmg_name1', 'fmg_name2', 'fi_name', 'cveg_name',
  'climate__micro', 'unc_cveg_sugarcane'
))])
...
for (x in c(
  socref_name, flu_name1, flu_name2, fmg_name1, fmg_name2, fi_name
)) {
  load(
    file.path(
...
for (i in 1:nrow(climate__micro_temp)) {
  ...
  dist_flu <- 
    unc_factors__sugarcane$`Set aside` * (
      flu_name1 %>% 
        get() %>% 
        ...
        t() %>% 
        rowSums()
    ) + unc_factors__sugarcane$`Annual cropland` * (
      flu_name2 %>% 
        get() %>% 
        magrittr::use_series("samples") %>% 
        apply(1, function(x, w){x * w}, 
  ...
  # If IBGE code > 20000 and IBGE code < 30000
  # FI = Weighted FI Low Weighted;
  # else 
  # FI = FI High without manure.
  if (climate__micro_temp$IBGE_code[i] > 20000 & 
      climate__micro_temp$IBGE_code[i] < 30000) {
  ...
  # Default uncertainty
  unc <- unc_cveg_sugarcane %>% 
    magrittr::use_series("uncertainty_cveg_based_on_annual_fluctuations") %>% 
    magrittr::extract(i) %>% 
...
rm(dist_socref, dist_flu, dist_fmg, dist_fi, dist_soc, dist_cveg, dist_total,
  socref_name, flu_name1, flu_name2, fmg_name1, 
  fmg_name2, fi_name, cveg_name
)
