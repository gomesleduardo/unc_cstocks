...
# SETTINGS --------------------------------------------------------------------
type_name <- 'tc__grassland_cultivated_average_management_2022_medium'
socref_name <- 'samples__socref'
flu_name <- 'samples__grassland_land_use_flu_grassland'
fmg_name1 <- 'samples__grassland_management_fmg_moderately_degraded'
fmg_name2 <- 'samples__grassland_management_fmg_severely_degraded'
fmg_name3 <- 'samples__grassland_management_fmg_nominally_managed_non_degraded'
fi_name <- 'samples__grassland_input_applied_only_to_improved_grassland_fi_medium'
cveg_name <- 'grasslands'
...
rm(list = ls()[!(ls() %in% c(
  ...
  'fmg_name1', 'fmg_name2', 'fmg_name3',
  'fi_name', 'cveg_name', 'climate__micro', 'unc_cveg', 'degradation_df'
))])
...
for (x in c(socref_name, flu_name, fmg_name1, fmg_name2, fmg_name3, fi_name)) {
  load(
    file.path(
...
for (i in 1:nrow(climate__micro_temp)) {
  ...
  dist_fmg <- 
    degradation_df$intermediario_2022[i] * (
      fmg_name1 %>% 
        get() %>% 
        magrittr::use_series("samples") %>% 
        apply(1, function(x, w){x * w}, 
...
rm(dist_socref, dist_flu, dist_fmg, dist_fi, dist_soc, dist_cveg, dist_total,
   socref_name, flu_name, fmg_name1, fmg_name2, fmg_name3, fi_name, cveg_name)
