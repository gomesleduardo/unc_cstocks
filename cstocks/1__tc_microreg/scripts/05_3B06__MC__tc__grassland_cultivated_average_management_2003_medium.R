...
# SETTINGS --------------------------------------------------------------------
type_name <- 'tc__grassland_cultivated_average_management_2003_medium'
...
for (i in 1:nrow(climate__micro_temp)) {
  ...
  dist_fmg <- 
    degradation_df$intermediario_2003[i] * (
      fmg_name1 %>% 
        get() %>% 
        magrittr::use_series("samples") %>% 
        apply(1, function(x, w){x * w}, 
