...
# SETTINGS --------------------------------------------------------------------
type_name <- 'tc__cropland_temporary_generic_average_tillage_2006_medium'
...
for (i in 1:nrow(climate__micro_temp)) {
  ...
  dist_fmg <- 
    area_ocupped_df$full_tillage_2006[i] * (
      fmg_name1 %>% 
        get() %>% 
        magrittr::use_series("samples") %>% 
        apply(1, function(x, w){x * w}, 
        ...
        t() %>% 
        rowSums()
    ) + area_ocupped_df$no_till_2006[i] * (
      fmg_name2 %>% 
        get() %>% 
        magrittr::use_series("samples") %>% 
        apply(1, function(x, w){x * w}, 
        ...
        t() %>% 
        rowSums()
    ) + area_ocupped_df$reduced_tillage_2006[i] * (
      fmg_name3 %>% 
        get() %>% 
        magrittr::use_series("samples") %>% 
        apply(1, function(x, w){x * w}, 
