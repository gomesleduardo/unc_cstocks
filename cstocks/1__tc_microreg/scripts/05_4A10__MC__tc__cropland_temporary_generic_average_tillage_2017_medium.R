...
# SETTINGS --------------------------------------------------------------------
type_name <- 'tc__cropland_temporary_generic_average_tillage_2017_medium'
socref_name <- 'samples__socref'
flu_name <- 'samples__cropland_land_use_flu_annual_cropland'
fmg_name1 <- 'samples__cropland_tillage_fmg_full'
fmg_name2 <- 'samples__cropland_tillage_fmg_no_till'
fmg_name3 <- 'samples__cropland_tillage_fmg_reduced'
fi_name <- 'samples__cropland_input_f1_medium'
cveg_name <- 'anual_croplands'
...
rm(list = ls()[!(ls() %in% c(
  ...
  'fi_name', 'cveg_name', 'climate__micro', 'unc_cveg', 'area_ocupped_df'
))])
...
for (i in 1:nrow(climate__micro_temp)) {
  ...
  dist_fmg <- 
    area_ocupped_df$full_tillage_2017[i] * (
      fmg_name1 %>% 
        get() %>% 
        magrittr::use_series("samples") %>% 
        apply(1, function(x, w){x * w}, 
        ...
        t() %>% 
        rowSums()
    ) + area_ocupped_df$no_till_2017[i] * (
      fmg_name2 %>% 
        get() %>% 
        magrittr::use_series("samples") %>% 
        apply(1, function(x, w){x * w}, 
        ...
        t() %>% 
        rowSums()
    ) + area_ocupped_df$reduced_tillage_2017[i] * (
      fmg_name3 %>% 
        get() %>% 
        magrittr::use_series("samples") %>% 
        apply(1, function(x, w){x * w}, 
